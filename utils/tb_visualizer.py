...
class TBVisualizer:
    ...
    # def display_current_results2(self, visuals, it, is_train, save_visuals=False):

    #     for label, image_numpy in visuals.items():
    #         sum_name = '{}-{}'.format('Train-Fake' if is_train else 'Test-Fake', label)
    ...
    #                                          'event_imgs-Fake', '%08d' % it + '-' + sum_name +'.png'))

    #     self._writer.export_scalars_to_json(self._tb_path)

    def display_current_results(self, visuals, epoch, it, is_train, save_visuals, flag):

        for label, image_numpy in visuals.items():
            sum_name = '{}-{}'.format('Train_' + flag if is_train else 'Test_' + flag, label)
            
            self._writer.add_image(sum_name, image_numpy, it)
            if save_visuals:
                util.save_image(image_numpy,
                                os.path.join(self._opt.checkpoints_dir, self._opt.name, 
                                             'event_imgs_' + str(epoch), flag, '%08d' % it + '-' + sum_name +'.png'))

        self._writer.export_scalars_to_json(self._tb_path)

    def display_test_current_results(self, visuals, epoch, it, is_train, save_visuals, flag):
        ...
        #     # self._writer.add_image(sum_name, image_numpy, it)
        #     if save_visuals:
        util.save_image(visuals['4_batch_fake_img_synthesis'],
                        os.path.join(self._opt.checkpoints_dir, self._opt.name, 
                                        'event_imgs_' + str(epoch), flag))
